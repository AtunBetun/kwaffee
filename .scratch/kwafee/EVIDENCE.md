...
## 2026-09-10 — v2 first-person co-op pivot (ops, owner-directed)

- Owner directed a full restart: delete ALL Beads and COMPLETELY clear the Unity codebase; first-person game; guns in the loop; co-op (shared objective + money); extremely simple start UI; asset-store free assets ("just wire up code"); physics-engine hunt to delete beads; "How To Fish style"; coordinator takes all decisions ("work by yourself — I do not want you asking me questions"); create ADRs + plans as Beads.
- Research completed (scout subagents, primary sources): PHYSICS — keep Unity built-in Rigidbody, no third-party engine exists that swaps GameObject physics in-place; 30Hz authoritative tick is the pattern; WebGL-safe; ragdolls on Rigidbody+joints adequate (`.scratch/kwafee/research/physics-engines.md`). NETCODE — FishNet recommended (free, no CCU, `PredictionRigidbody` + TimeManager at 30Hz maps 1:1 onto ADR-0001, headless dedicated server, WebGL client via WSS, active 4.7.3R Sep 2026, ~9.4k Discord); PurrNet runner-up; Photon Fusion (CCU lock-in) and NGO+Relay (no client prediction) rejected (`.scratch/kwafee/research/netcode-2026.md`).
- Asset Store verified by driving the live store search UI (`#q=<q>&nf-ec_price_filter=0...0`): Tier-S free listings confirmed incl. First Person + Third Person | Character Controllers (196526), Low Poly Shooter Pack - Free Sample (144839), Low Poly Pistol/Shotgun packs (285693/283665), Simple Crosshair Generator (160160), DOTween (27676), In-game Debug Console (68068), Cartoon FX Remaster Free (109565), Footsteps - Essentials (189879), Stylized Low Poly Coffee Shop Props Pack - Free (368786), Food/Furniture/Weapons FREE packs (260726/260522/260492), Fish - PolyPack (202232), Low Poly Nature/Street/Tree packs; unverified-price items flagged in `.scratch/kwafee/research/assets-2026.md` (install-time human confirmation per ADR-0015).
- Wipe executed: Beads export backed up (`.scratch/kwafee/backups/beads-export-20260910-preWipe.jsonl`, 51 rows, 75 dep links) then all issues deleted (bd remaining=0); v1 Unity authored code removed; `ProjectSettings/`/`Packages/`/`manifest.json` retained; v1 specs/grilling/plans archived to `.scratch/kwafee/backups/v1/`; commits `40e0ca6` (snapshot) and `9f678a5` (wipe), tree clean after.
- Decisions recorded as ADR-0010 (pivot), 0011 (co-op first), 0012 (guns & ghetto violence), 0013 (keep Rigidbody physics), 0014 (FishNet), 0015 (asset-store supply), 0016 (minimal UI) at `docs/adr/`.
- Design authored: `.scratch/kwafee/v2-design.md` — fantasy, shift anatomy, true first-person presentation, guns/intimidation/shootout model, shared till economy, ragdolls, space, session flow, NPC cast, technical spine, and the 14-spec v2 list each with a Refined-Gate direction.
- 14 v2 spec writers dispatched in one parallel batch (2026-09-10); specs land at `.scratch/kwafee/specs/<NN>-<slug>-spec.md`. Next: verify specs, to-tickets → recreate Beads.
