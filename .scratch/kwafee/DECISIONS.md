...
## v2 pivot — 2026-09-10 (owner-directed restart; SUPERSEDES the v1 sections above where they conflict)

- **Wipe:** all v1 Beads issues deleted (backup: `.scratch/kwafee/backups/beads-export-20260910-preWipe.jsonl`, 51 rows) and all v1 authored Unity code deleted (`kwaffee/Assets/KwaFee`, `Scenes`, `TutorialInfo`, `Screenshots`, v1 InputSystem_Actions, stale folder metas; `kwaffee/kwaffee.slnx` reduced to empty solution shell). `ProjectSettings/` + `Packages/` + `manifest.json` kept so Unity Hub reopens the project without human re-setup. Snapshots: `40e0ca6` (pre-wipe) + `9f678a5` (wipe). v1 specs/grilling/plans archived at `.scratch/kwafee/backups/v1/`. All of it is by design: "all of that codebase is useless, we are starting from scratch."
- **Game jid:** first-person co-op multiplayer party game — each player is TRULY first person (FPS look/sprint/crouch/jump + hands rig), 3-4 friends run the ghetto coffee shop together. How To Fish lens + Gamble With Your Friends pacing (ADR-0010).
- **Co-op first:** shared quota (serve N orders per shift), shared till (one wallet; tips + extortion land there), upgrades bought by anyone; per-player stats feed the awards only — NO per-player currency, no theft economy (ADR-0011).
- **Guns ARE in the game** (owner: "I DO want guns"): pistol/SMG/shotgun from free store packs; shoot customers/friends/machines; **intimidation** = aim at a customer → they hand over cash or their coffee; armed NPCs return fire; random **ghetto shootout events** (drive-by/walk-in) where everyone can die; comedy violence guardrails (ADR-0012).
- **Physics = Unity built-in Rigidbody**, server-authoritative fixed 30Hz tick, no third-party engine (research verdict, ADR-0013). The v1 custom physics-verb stack is the "bunch of beads" we deleted — physics was never the differentiator, the wiring is.
- **Netcode = FishNet** (free, no CCU, Unity 6 + WebGL via WSS, headless dedicated server, `PredictionRigidbody` at 30Hz maps onto ADR-0001 exactly); PurrNet runner-up; Photon/NGO rejected (ADR-0014).
- **Asset strategy:** source everything replaceable from the FREE Unity Asset Store (verified catalog: `.scratch/kwafee/research/assets-2026.md`); game code sits on our own seams behind adapters; one movement authority (Unity Starter Assets FirstPerson); Blender bpy pipeline only for signature pieces (machines, cat, sign) (ADR-0015). Human confirms each install.
- **Minimal UI:** ONE screen to start (name → room code → START), thin event-driven HUD (till/quota/clock/order board/crosshair/ammo); DOTween; no menus/inventory screens (ADR-0016).
- **Five verbs (v2):** MOVE/LOOK, INTERACT (E raycast), FLING (hold-E charge throw), SHOOT (LMB), WORK (the shift itself). "Verbs" = actions, NOT speaking; voice chat is explicitly out of v2 scope (future: Photon Voice 2).
- **Master design:** `.scratch/kwafee/v2-design.md` (fantasy, loop anatomy, guns model, shared economy, ragdolls, space, session flow, NPC cast, technical spine, the 14-spec list + per-spec gate direction). It replaces v1 design/architecture spec.
- **14 v2 specs** being written (to-spec template + Refined Gate each) at `.scratch/kwafee/specs/<NN>-<slug>-spec.md`; then to-tickets → Beads recreation. Blocking graph = specs 01→02→03→04→05→06 core chain; 07/08 enrich; 09/11 multiplayer foundations parallel; 10/12/13/14 layer on top.
- **Human prereqs unchanged:** Unity Personal license + WebGL Build Support (Hub) still block batchmode/WebGL builds; Asset Store sampling needs human installs; never retry batchmode licensing.
- Round structure carries over where free: 3-4 min shifts, Big Tony fixture loss, catastrophe replay, awards, tabloid, Boston voice. v1's custom CoreGame/VerbRules/RoundRules/SimHarness architecture is retired; new seams per v2 specs (PlayerMotor, PlayerLook, PlayerInteraction, HealthComponent, WeaponController, PhysicsGrabber, ShiftManager, ShootoutDirector, RagdollController, AudioManager, LobbyUI, HUD).
