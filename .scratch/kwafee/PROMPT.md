...
> THIS FILE IS THE PROMPT. A fresh BUILDER session starts by reading this entire file, then `/DECISIONS.md` and `/EVIDENCE.md` in this same folder. RATER sessions read this file, review the current artifact, and append their verdict to `/EVIDENCE.md`. Nothing else is needed to resume work.

## ENVIRONMENT — THE MACHINE

- Repo: this working directory. Memory lives in `.scratch/kwafee/` (this file, DECISIONS.md, EVIDENCE.md).
- Blender CLI on PATH as `blender` (currently 5.2.1 LTS). Headless ONLY: `blender -b -P script.py`. Never open the GUI.
- Unity editor installed with an activated Personal license and WebGL Build Support (a human completed these). Unity project lives at `/Users/albertodesaintmalo/Kwafee` — verify the absolute path yourself, create it if missing, record it in DECISIONS.md. If a build fails with a licensing error: STOP, record it, never retry in a loop.
- Model tiers: YOUR session runs on the cheap builder model (DeepSeek V4 Flash). RATER sessions run on GPT 6 Astra on a codex subscription. You never pick the expensive model; the human routes RATER sessions separately.
- Budget reality: the expensive tier is scarce. Do the bulk of iterations cheap. Never send an artifact to RATER before the /sim numbers are recorded and your own self-review pass is done.
