...
- **Frontier liveness:** `bd ready` excludes `in_progress` issues, and the strict artifact chain means only ~2 tickets are ever simultaneously claimable (the shift-seam plus one chain head). To keep the loop from starving after ~2 iterations, the loop RELEASES the claim on a clean-exit no-promise iteration (`bd update <id> --status open --assignee ""`), returning the ticket to `bd ready` for a fresh attempt. A `<promise>FAILED</promise>` ticket stays claimed for human triage; an omp crash retries the same ticket next iteration. This is the loop-contract rule, not a one-off fix — do not "improve" it back into claim-forever.
...
- **Epics carry no block edges.** The connectivity that matters is entirely in the children (cross-artifact + seam gates); epic↔epic block edges are redundant and were removed. Grooming dep edges in batches requires a fresh `bd export` first and MUST NOT run while an `afk-loop.sh` process is alive — both processes write the same Dolt working set, and a mid-batch node loss cascaded into a vanished ticket (see DECISIONS.md incident 2026-09-10).